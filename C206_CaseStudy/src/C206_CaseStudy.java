...
public class C206_CaseStudy {
		...
		while (option != 6) {
			...
			if (option == 2) {
				...
				while (a_option != 4) {
					...
					if (a_option == 1) {
						CourseCategory cc = inputCourseCategory(categoryList);
						...
						C206_CaseStudy.viewCourseCategory(categoryList);
						String deleteCategory = Helper.readString("Enter category name to delete > ");
						C206_CaseStudy.deleteCourseCategory(categoryList, deleteCategory);
					} else if (a_option == 4) {
						System.out.println("Thank you!");
				}
			...
			if (option == 3) {
				...
				while (c_option != 4) {
					...
					c_option = Helper.readInt("Select Option > ");
			...
			if (option == 4) {
				...
				while (b_option != 6) {
					...
					b_option = Helper.readInt("Selection Option > ");
	...
	public static CourseCategory inputCourseCategory(ArrayList<CourseCategory> categoryList) {
	...
	public static void addCatogory(ArrayList<CourseCategory> categoryList, CourseCategory cc) {
		
		categoryList.add(cc);
		System.out.println("Catagory added");
	}
	...
	public static void viewCourseCategory(ArrayList<CourseCategory> categoryList) {
		...
		output += retrieveAllCourseCategory(categoryList);
		System.out.println(output);
	}
	
	public static String retrieveAllCourseCategory(ArrayList<CourseCategory> categoryList) {
		String output = "";
		for (CourseCategory category : categoryList) {
	...
	public static void deleteCourseCategory(ArrayList<CourseCategory> categoryList, String deleteCategory) {
	...
	public void ViewCourse() {
		...
		output += String.format("%-12s %-12s %-14s %-50s %-15s %-20s\n", "coursecode", "title", "categoryname",
				"description", "duration", "courseprerequisite");

		for (Course i : courseList) {
			output += String.format("%-12s %-12s %-14s %-50s %-15s %-20s\n", i.getcoursecode(), i.gettitle(),
					i.getcategoryname(), i.getdescription(), i.getduration(), i.getcourseprerequisite());
		}
		System.out.println(output);
	}
...
//		System.out.println("Course Schedule Added");
	...
	public static String retrieveAllCourseSchedule(ArrayList<CourseSchedule> scheduleList) {
		...
		for (CourseSchedule schedule : scheduleList) {
			output += String.format("%-10d %-10.2f %-20s %-20s %-10s", schedule.getId(), schedule.getPrice(),
					schedule.getstartDateTime(), schedule.getendDateTime(), schedule.getLocation());
		}
		return output;
	}
	...
	public static void searchCourseSchedule(ArrayList<CourseSchedule> scheduleList, int id) {
		for (int i = 0; i < scheduleList.size(); i++) {
			if (id == scheduleList.get(i).getId()) {
				...
				output += String.format("%-10d %-10.2f %-20s %-20s %-10s", scheduleList.get(i).getId(),
